...
# ved et sigifikantnivå under 5% observere vi pilsner og non-alcoholic.
# Pilsner selger mest og non-alcoholic dårligst. 
#  
...
Bukta_data %>% 
  ...
  ggplot(aes(per15min, inntekt, color = ar)) +
  geom_line(size = 1)+
  facet_wrap(~dag)
...
Regresjon2 <-  lm(inntekt ~ nedbør + temp + solskin + vind + gjester + Dag + Ar, data = Per15)
summary(Regresjon2)
